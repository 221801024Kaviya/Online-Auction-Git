...
const AuctionDetails = () => {
  ...
  useEffect(() => {
    const fetchAuctions = async () => {
      try {
        const response = await fetch("http://localhost:5000/api/auctions"); // Fetch all auctions
...
//       // 🔄 Fetch latest auctions after placing bid
//       fetchAuctions();  
//     } else {
        ...
        {auctions.length > 0 ? (
          auctions.map((auction) => (
            <div key={auction._id} className="auction-card">
              ...
              {/* <p><strong>Current Bid:</strong> ₹{auction.currentBid !== undefined ? auction.currentBid : auction.amount}</p> */}
              <p><strong>Current Bid:</strong> ₹{auction.currentBid || auction.amount}</p>
